...
if __name__ == '__main__':
    ...
    https_host = "https://httpbin.org/anything"
    timeout = 1
    #
    #
    try:
        response_https = requests.get(url=https_host, headers=headers, proxies=proxy_input, timeout=timeout)
        if response_https.status_code == 200:
            print('\n')
            print(response_https.text)
    except:
        print(f"\t[http] '{proxy}' problemli olabilir!" )
